...
namespace SistemaVenta.BLL.Servicios
{
    public class DashBoardService :IDashBoardService
    {
        ...
        {
            DateTime? ultimaFecha=tablaVenta.OrderByDescending(y=>y.FechaRegistro).Select(v=>v.FechaRegistro).First();
        ...
        private async Task<Dictionary<string,int>> VentasUltimaSemana()
        {
            ...
            if (_ventaQuery.Count() > 0)
            {
                ...
                resultado=tablaVenta.GroupBy(v=>v.FechaRegistro.Value.Date).OrderBy(g=>g.Key).Select(dv=>new { fecha=dv.Key.ToString("dd/MM/yyyy"),total=dv.Count()})
                    .ToDictionary(keySelector:a=>a.fecha,elementSelector:a=>a.total);
